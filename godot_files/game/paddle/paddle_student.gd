...
@export var q_table_path: String = "res://models/q_table_student.json"
...
func _load_q_table() -> void:
	assert(FileAccess.file_exists(q_table_path), "PaddleStudent: Q-table not found at: " + q_table_path)

	var file: FileAccess = FileAccess.open(q_table_path, FileAccess.READ)
	var json: JSON = JSON.new()
	var parse_result: int = json.parse(file.get_as_text())
	assert(parse_result == OK, "PaddleStudent: Failed to parse Q-table JSON: " + json.get_error_message())
	...
	print("PaddleStudent: loaded %d entries, state_vars=%s, bins=%s" % [q_table.size(), state_vars, bins])
	file.close()
...
func get_direction() -> int:
	...
	# Y coordinates do not need mirroring (vertical axis is unchanged).
	...
	# Reproduce Python's str(tuple) format: "(4, 0, 1, 1, 1, 1)"
